...
public class DbUtils {
	...
	public static final String T_RESULTSET = WRAP + SYSTEM_PREFIX + "resultset"
			+ WRAP;
	public static final String T_NOTIFY = WRAP + SYSTEM_PREFIX + "notify"
			+ WRAP;
	...
	public static final String T_MANAGEMENT = WRAP + SYSTEM_PREFIX
			+ "management" + WRAP;
	...
	/**
	 * Logga le operazioni su database dell'utente. Ogni messaggio sarà
	 * preceduto dall'identificativo dell'utente
	 * 
	 * @param message
	...
	public void updateUserProperties(User user) throws HiddenException {
		Connection connection = dbConnectionHandler.getConn();
		String query = "UPDATE " + T_USER + " SET" + " password=PASSWORD('"
	...
	public List<Integer> getUserResultsetHeaderPrefereces(Integer uid,
			Integer rsid) {
		String query = "SELECT fip.id_field as fieldid FROM " + "(((" + T_FIELD
				+ " f JOIN " + T_FIELDINPREFERENCE + " fip "
				+ "ON (fip.id_field=f.id)) " + "JOIN " + T_HEADERPREFERENCE
				+ " hp ON " + "(hp.id=fip.id_headerpreference)) " + "JOIN "
				+ T_USER + " u " + "ON (hp.id_user=u.id)) " + "WHERE u.id = '"
				+ uid + "' AND f.id_resultset='" + rsid + "'";
	...
	private String createSearchQuery(PagingLoadConfig config,
		...
		for (String key : fields.keySet()) {
			...
			if (value.length() > 0) {
				...
				if (key.compareTo(SPECIAL_FIELD) != 0) {
					...
					while (stringTokenizer.hasMoreTokens()) {
						String token = stringTokenizer.nextToken();
						query += fieldTest(dbProperties.getFieldList(id), "OR",
								token, like);
					}
				}
		...
		if (config != null) {
			if (config.getSortInfo().getSortField() != null) {
				query += " ORDER BY `" + config.getSortInfo().getSortField()
						+ "` " + config.getSortInfo().getSortDir();
			}
			query += " LIMIT " + ((PagingLoadConfig) config).getOffset() + ","
	 ...
	 */
	private String fieldTest(List<String> fields, String operation,
	...
	/**
	 * Esegue una query su database e restituisce la lista dei record sotto
	 * forma di List<BaseModelData>. Le proprietà del ModelData sono inviate
	 * tramite BaseModelData vedi:
	 * http://extjs.com/deploy/gxtdocs/com/extjs/gxt/ui/client
	...
	public int countObjects(SearchParams searchParams) throws HiddenException {
		...
		try {
			String query = "SELECT COUNT(*) FROM ("
					+ createSearchQuery(null, searchParams) + ") as A";
	...
	public List<BaseModelData> getReGroupings(int resultSetId)
		...
		String groupingQuery = "SELECT DISTINCT " + T_GROUPING + ".id as id, "
				...
				+ ".alias as alias " + "FROM (" + T_GROUPING + " JOIN "
				+ T_FIELD + " ON " + T_GROUPING + ".id = " + T_FIELD
				+ ".id_grouping) " + "WHERE " + T_FIELD + ".id_resultset = '"
				+ resultSetId + "'";
	...
	private List<BaseModelData> getValuesOfAField(String resultset,
		...
		try {
			String query = "SELECT DISTINCT `" + fieldName + "` FROM "
					+ resultset + " ORDER BY `" + fieldName + "` ASC";
	 ...
	 * @throws HiddenException
	 */
	public List<BaseModelData> getValuesOfAField(int resultsetId,
	 ...
	 *         tabella. Serve a riempire i combobox per la modifica della
	 *         griglia
	 * @throws HiddenException
	 * @throws SQLException
	 */
	public List<BaseModelData> getValuesOfAFieldFromTableName(String tableName,
		...
		try {
			...
			e.printStackTrace();
			throw new HiddenException(
					"Errore con i valori della chiave primaria "
	...
	public List<Message> getUserMessages(int uid) throws HiddenException {
		...
		String query = "(SELECT m.* FROM " + T_MESSAGES + " m JOIN " + T_USER
				+ " u "
		...
		try {
			...
			while (result.next()) {
				...
				Message w = new Message(id, title, body, date, type, sender,
						recipient);
				messages.add(w);
			}
		} catch (SQLException e) {
	...
	public int setObjects(Integer resultsetId, List<BaseModelData> records)
		...
		try {
			...
			if (ex.getErrorCode() == 1062) {
				...
				newMess = newMess.concat(ex.getErrorCode()
	 ...
	 * tableName + "` WHERE ";
	 * 
	 * for (String property : record.getPropertyNames()) { query += "`" +
	 * property + "`=?" + sep; } query = query.substring(0, query.length() -
	 * sep.length());
	 * 
	...
	public Integer removeObjects(Integer resultsetId,
			List<BaseModelData> records) throws HiddenException {
		...
		try {
			...
			List<BaseModelData> primaryKeyList = dbProperties
			...
			Log.warn("Errore SQL", e);
			throw new HiddenException(
					"Errore durante l'eliminazione dei record");
		} finally {
	...
	private String getForeignKeyForAField(String fieldName, ResultSet result)
		...
		for (int i = 1; i <= result.getMetaData().getColumnCount(); i++) {
			if (result.getMetaData().getColumnLabel(i).compareToIgnoreCase(
					fieldName) == 0) {
	...
	public User getUser(Credentials credentials) throws VisibleException {
		...
		try {
			...
			throw new VisibleException(
					"Errore durante l'interrogazione su database");
		}
		...
		try {
			while (result.next()) {
				...
				List<ResultsetImproved> resultsets = getUserResultsetImproved(
						uid, gid);
				...
				User user = new User(uid, gid, new Credentials(username,
						password), name, surname, group, email, office,
						telephone, status, login, last, resultsets, messages);
				this.user = user;
				return user;
			}
		} catch (Exception e) {
		...
		throw new VisibleException(
				"Errore di accesso: username o password errati");
	}

	/**
	 * @param resultsetId
	 * @return Crea una fieldMatrix che contiene i valori di autocompletamento
	 *         per ogni campo del resultset passato per parametro. I valori di
	...
	public FieldsMatrix getValuesOfFields(Integer resultsetId)
		...
		try {
			...
			for (Integer fieldId : rsf.keySet()) {
				List<BaseModelData> autoCompleteList = getValuesOfAField(
						resultsetId, rsf.get(fieldId));
	...
	private HashMap<Integer, String> getResultsetFields(int resultsetId)
		...
		Connection connection = dbConnectionHandler.getConn();
		String query = "SELECT res.id as id, res.name as name FROM "
				+ T_RESOURCE + " res JOIN " + T_FIELD
	...
	public FieldsMatrix getValuesOfForeignKeys(Integer resultsetId)
		...
		try {
			...
			String query = "SELECT * FROM "
					+ dbProperties.getStatement(resultsetId) + " WHERE 0";
			ResultSet resultset = doQuery(connection, query);

			for (Integer fieldId : rsf.keySet()) {
				String foreignKey = getForeignKeyForAField(rsf.get(fieldId),
						resultset);
				String fkTName;
				String fkFName;
				List<BaseModelData> autoCompleteList = new ArrayList<BaseModelData>();
				if (foreignKey != null && foreignKey.length() > 0) {
					...
					autoCompleteList = getValuesOfAFieldFromTableName(fkTName,
							fkFName);
	...
	public ArrayList<IncomingForeignKeyInformation> getForeignKeyInForATable(Integer resultsetId)
		...
		String query = "SELECT statement FROM " + T_RESULTSET + " WHERE id = '"
				+ resultsetId + "'";
		try {
			ResultSet res = doQuery(connection, query);
			while (res.next()) {
			...
			Connection connectionInformationSchema = dbConnectionHandler
					.getConnDbInformationSchema();
			String queryFKIn = "SELECT TABLE_NAME, COLUMN_NAME, REFERENCED_COLUMN_NAME FROM KEY_COLUMN_USAGE where TABLE_SCHEMA = '"
					+ db + "' AND REFERENCED_TABLE_NAME = '" + tableName + "'";
			ResultSet resultFKIn = doQuery(connectionInformationSchema,
					queryFKIn);
	...
	public List<ResultsetImproved> getUserResultsetImproved(Integer uid,
		...
		try {
			...
			while (result.next()) {
				...
				Integer resultsetid = Integer.valueOf(result
						.getInt("resultsetid"));
				boolean defaultheader = result.getBoolean("defaultheader");
				Integer searchgrouping = Integer.valueOf(result
						.getInt("searchgrouping"));
				Integer idgrouping = Integer.valueOf(result
						.getInt("idgrouping"));
				...
				if (statement != null) {
					...
					for (BaseModelData grouping : groupings) {
						ResultsetFieldGroupings rfg = new ResultsetFieldGroupings(
								(Integer) grouping.get("id"), (String) grouping
										.get("name"), (String) grouping
										.get("alias"));
					...
					ResultsetField resField = new ResultsetField(id, name,
							...
							idgrouping, readperm, deleteperm, modifyperm,
							insertperm, visible);
	...
	public boolean setUserResultsetHeaderPreferencesNoDefault(Integer userid,
		...
		String createHeaderPreference = "INSERT INTO " + T_HEADERPREFERENCE
				+ " VALUES  (NULL , '" + value + "-" + creationDate.hashCode()
				+ "', '" + userid + "', '" + creationDate + "')";
		...
		try {
			...
			for (int i = 0; i < listfields.size(); i++) {
				String newPreferences = "INSERT INTO " + T_FIELDINPREFERENCE
						+ " VALUES ('" + headerPrefId + "', '"
						+ listfields.get(i) + "')";
	...
	public int importFile(Credentials credentials, int resultsetId,
		...
		try {
			...
			e.printStackTrace();
			throw new HiddenException("file " + importFile.getName()
					+ " non trovato");
		}
		...
		try {
			...
			e.printStackTrace();
			throw new HiddenException(
					"impossibile recuperare metadati per resultset: "
							+ resultsetId);
		}
	...
	private BaseModelData createRecord(ResultSetMetaData rsmd, String recordLine)
		...
		try {
			...
			e.printStackTrace();
			throw new HiddenException(
					"il seguente record è valido, ma non riesco a crearlo: "
							+ recordLine);
		}
	...
	private boolean validateLine(ResultSetMetaData rsmd, String recordLine)
		...
		try {
			...
			e.printStackTrace();
			throw new HiddenException(
					"problemi nella lettura dei metadata per il rs");
		}
	...
	public Integer updateObjects(Integer resultsetId,
		...
		try {
			...
			for (BaseModelData record : newItemList) {
				...
				for (BaseModelData pk : PKs) {
					PKset = PKset.concat((String) pk.get("PK_NAME") + "="
	...
	public void notifyChanges(Integer resultsetId,
			List<BaseModelData> newItemList) throws SQLException,
			HiddenException {
		Integer id_table = 0;
		...
		while (result.next()) {
			...
			for (BaseModelData record : newItemList) {
				if (record.get(bmdid) != null) {
					...
					PreparedStatement psData = (PreparedStatement) connection
							.prepareStatement(data_statement);
					...
					PreparedStatement ps = (PreparedStatement) connection
							.prepareStatement(address_statement);
						...
						if (!(resultAddress.getString(1) == null)) {
							try {
								MailUtility.sendMail(
										resultAddress.getString(1), mitt,
										oggetto, testo);
