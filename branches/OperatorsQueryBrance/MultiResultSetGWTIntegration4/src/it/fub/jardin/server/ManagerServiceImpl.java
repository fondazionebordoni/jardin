...
public class ManagerServiceImpl extends RemoteServiceServlet implements
	...
	public String createReport(String file, Template template,
		...
		String xsl = this.getServletContext().getRealPath(
		...
		if (template != null) {
			...
			String result = FileUtils.createReport(realpath, xsl, template,
					records, columns, fs, ts);
	...
	public PagingLoadResult<BaseModelData> getRecords(PagingLoadConfig config,
		...
		if (config != null) {
			...
			return new BasePagingLoadResult<BaseModelData>(records, config
					.getOffset(), recordSize);
		} else {
	...
	public Integer updateObjects(Integer resultsetId,
		...
		// e passaggio a notifyCanges
		List<BaseModelData> newItemListTest = newItemList;
		int success = dbUtils.updateObjects(resultsetId, newItemList, condition);
		if (success > 0) {
			try {
				dbUtils.notifyChanges(resultsetId, newItemListTest);
			} catch (SQLException e) {
